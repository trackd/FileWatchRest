...
public class ExternalConfigurationValidatorEdgeCasesTests {
    ...
    public void EmptyApiEndpointFailsValidation() {
        // Create a RestPost action without per-action ApiEndpoint so top-level ApiEndpoint
        // being empty should cause validation to fail.
        var config = new ExternalConfiguration {
            Folders = [new ExternalConfiguration.WatchedFolderConfig { FolderPath = "C:\\temp", ActionName = "rest" }],
            Actions = [new ExternalConfiguration.ActionConfig { Name = "rest", ActionType = ExternalConfiguration.FolderActionType.RestPost }],
