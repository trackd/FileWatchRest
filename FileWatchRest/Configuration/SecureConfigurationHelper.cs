...
public static class SecureConfigurationHelper {
    ...
    public static string EncryptBearerToken(string plainTextToken) {
        ...
        try {
            byte[] plainTextBytes = Encoding.UTF8.GetBytes(plainTextToken);
            byte[] encryptedBytes = ProtectedData.Protect(plainTextBytes, AdditionalEntropy, DataProtectionScope.LocalMachine);
    ...
    public static string DecryptBearerToken(string encryptedToken) {
        ...
        if (!encryptedToken.StartsWith(EncryptedTokenPrefix, StringComparison.Ordinal)) {
            throw new InvalidOperationException("Token does not have the expected encryption prefix");
        }

        try {
            string base64Token = encryptedToken[EncryptedTokenPrefix.Length..];
            byte[] encryptedBytes = Convert.FromBase64String(base64Token);
            byte[] decryptedBytes = ProtectedData.Unprotect(encryptedBytes, AdditionalEntropy, DataProtectionScope.LocalMachine);
    ...
    public static bool IsTokenEncrypted(string? token) => !string.IsNullOrWhiteSpace(token) && token.StartsWith(EncryptedTokenPrefix, StringComparison.Ordinal);
